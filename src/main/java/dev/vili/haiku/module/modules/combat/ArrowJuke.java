...
public class ArrowJuke extends Module {
	public final ModeSetting modeM = new ModeSetting("Mode-ArrowJuke", "How to move to avoid the arrow", "Move", "Move", "Client", "Packet");
	public final NumberSetting speedN = new NumberSetting("Speed-ArrowJuke", "The move speed", 1, 0, 5, 0.01);
	public final NumberSetting lookahead = new NumberSetting("Lookahead-ArrowJuke", "How many steps in the future to look ahead",300, 250, 500, 1);
	public final BooleanSetting up = new BooleanSetting("Up-ArrowJuke", "Allows you to move up when dodging the arrow", false);

	public ArrowJuke() {
		super("ArrowJuke", "Tries to dodge arrows coming at you.", GLFW.GLFW_KEY_UNKNOWN, Category.COMBAT);
		this.addSettings(modeM, speedN, lookahead, up);
	}

	@HaikuSubscribe
	public void onTick(TickEvent envent) {
		for (Entity e : mc.world.getEntities()) {
			if (e.age > 75 || !(e instanceof ArrowEntity) || ((ArrowEntity) e).getOwner() == mc.player)
				continue;

			String mode = modeM.getMode();
			int steps = (int) lookahead.getValue();
