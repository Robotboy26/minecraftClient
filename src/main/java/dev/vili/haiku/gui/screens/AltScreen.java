...
import dev.vili.haiku.altmanager.Alt;
...
public class AltScreen extends Screen {
	...
	public List<Alt> getAltList() {
		return Haiku.getInstance().altManager.getAlts();
	}
	...
	public void loginToSelected() {
		...
		if (alt.isCracked()) {
			Haiku.getInstance().altManager.loginCracked(alt.getEmail());
		} else {
			Haiku.getInstance().altManager.login(alt);
		}
	}
	...
	public void deleteSelected() {
		...
		Haiku.getInstance().altManager.removeAlt(alt);
		this.refreshAltList();
	}
}
