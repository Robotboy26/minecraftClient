...
import dev.vili.haiku.utils.render.scan.Stores;
...
public class ScanController {
  ...
  private static boolean playerLocationChanged() {
    ...
    return playerLastChunk == null
        || playerLastChunk.x != currentChunk.x
        || playerLastChunk.z != currentChunk.z;
  }
  ...
  public static void runTask(boolean forceRerun) {
    ...
    if ((!forceRerun && !playerLocationChanged())) {
      return;
    }
  ...
  private static void findChunks() {
    ...
    int cX = playerChunkPos.x;
    int cZ = playerChunkPos.z;
