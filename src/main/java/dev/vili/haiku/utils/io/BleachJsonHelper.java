...
public class BleachJsonHelper {
		...
		if (BleachFileMang.fileExists(path)) {
			...
			if (!content.isEmpty()) {
				try {
					...
					HaikuLogger.logger.error("Error trying to read json file \"" + path + "\", Overwriting file to add element!", e);
				}
			}
		}
	...
	public static JsonObject readJsonFile(String path) {
		...
		try {
			...
			HaikuLogger.logger.error("Error trying to read json file \"" + path + "\", Deleting file!", e);
