package dev.vili.haiku.utils.thunderClient.utility.render;

import com.mojang.blaze3d.systems.RenderSystem;

import dev.vili.haiku.gui.font.Texture;
import dev.vili.haiku.module.modules.client.HudEditor;
import dev.vili.haiku.utils.thunderClient.utility.math.MathUtility;
import dev.vili.haiku.utils.thunderClient.utility.render.shaders.GradientGlowProgram;
import dev.vili.haiku.utils.thunderClient.utility.render.shaders.MainMenuProgram;
import dev.vili.haiku.utils.thunderClient.utility.render.shaders.RoundedGradientProgram;
import dev.vili.haiku.utils.thunderClient.utility.render.shaders.RoundedProgram;
...
public class Render2DEngine {
    ...
    public static Color skyRainbow(int[] is, int index) {
        int angle = (int) ((System.currentTimeMillis() / (long) is[index % is.length]) + index) % 360;
        return Color.getHSBColor((double) ((float) ((angle %= 360) / 360.0)) < 0.5 ? -((float) (angle / 360.0)) : (float) (angle / 360.0), 0.5F, 1.0F);
    }

    public static Color fade(int[] is, int index, Color color, float alpha) {
    ...
    public static Color getAnalogousColor(int[] is) {
        float[] hsb = Color.RGBtoHSB(is[0], is[1], is[2], null);
        float degree = 0.84f;
    ...
    public static Color rainbow(int[] is, int index, float saturation, float brightness, float opacity) {
        int angle = (int) ((System.currentTimeMillis() / is[index % is.length] + index) % 360);
    ...
    public static Object interpolateColorsBackAndForth(int[] rgbArray, int count, Color color, Color analogousColor,
            boolean b) {
        return null;
    }
}
