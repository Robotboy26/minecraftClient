...
public class DownloadUtils {
            ...
            while ((line = br.readLine()) != null) {
                String[] parts = line.split("/");
                    try {
                        ...
                        NebulaLogger.info("Downloaded mod! " + parts[parts.length - 1]);
                    } catch (Exception e) {
                        getMod(line, line, modsFolder);
                        NebulaLogger.info("Error while downloading mod from file: " + e.getMessage());
                    }
                }
        } catch (Exception e) {
    ...
    public static void getMod(String modName, String modUrl, String modsFolder) {
        if (!new File(modsFolder, modName).exists()) {
            ...
            NebulaLogger.info("Mod already exists! " + modName);
        }
    }
