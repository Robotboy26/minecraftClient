...
import dev.nebula.utils.altmanager.*;
import dev.nebula.utils.ListWidget;
import dev.nebula.utils.MultiProcessingUtils;
import dev.nebula.utils.json.JsonException;
import dev.nebula.utils.json.JsonUtils;
import dev.nebula.utils.json.WsonObject;
...
public final class AltManagerScreen extends Screen
{
	...
	private void pressImportAlts()
	{
		try
		{
			Process process = MultiProcessingUtils.startProcessWithIO(
				...
				Nebula.getInstance().getNebulaFolder().toString());
	...
	{
		// translate
		// String translated = Nebula.getInstance()
		// 	.translate("description.wurst.altmanager." + trKey);
		
		// line-wrap
		StringJoiner joiner = new StringJoiner("\n");
		textRenderer.getTextHandler().wrapLines("description.nebula.utils.altmanager." + trKey, 200, Style.EMPTY)
